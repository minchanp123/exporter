	...
	var process = function(sourcePath, customerKeywords, ret_num, callback){
		...
		async.eachSeries(customerKeywords, function(customerKeyword, callback) {
			...
			async.waterfall([
				...
				function(callback){
					logger.info(attachLoggerHeader(ret_num,'Step#4-2 DestPath 지정'));
					mariaDBDao.selectSearchKeywordInfo({search_keyword_text : keyword}, function(err, res){
						if (err){
							callback(err);
						}else{
							if(res.length == 0){
								 err = "ERR_NO_PROJECT_SEQ";//"no_project_seq";
								 callback(err);
							}else {
								var isTopicsY = false;
								...
								if (res.length > 1){
									for (var i=0; i<res.length; i++){
										if (res[i].topics_yn == 'Y'){isTopicsY = true;}
										if (res[i].emotions_yn == 'Y'){isEmotionsY = true;}
									}
								}else{
									if (res[0].topics_yn == 'Y'){isTopicsY = true;}
									if (res[0].emotions_yn == 'Y'){isEmotionsY = true;}
								}
								//파일 처리후 이동할 폴더 지정
								if (isTopicsY){	//화제어분석 진행시
									destPath = sourceTopicsPath;
								}else if (!isTopicsY && isEmotionsY){	//감성분석만 진행시 파일 이동
									destPath = sourceEmotionsPath
								}else{	//원문 insert 만 진행시(화제어 / 감성분석 진행 X)
						...
						async.waterfall([
							...
							function(callback) {
								...
								fs.readJson(json, function(err, docs) {
									...
									logger.info(attachLoggerHeader(ret_num,'Step#5-1 JSON 파일 객체로 변경 ' + docs.length));
									var skipIdxArr = [];
									var skipIdx = 0;
							...
							function(docs, callback) {
								logger.info(attachLoggerHeader(ret_num,'Step#5-2 원문 저장 ' + docs.length));
								save(docs, docsType, function(err){
									if (err){
										callback(err);
									} else {
										callback(null);
...
/*
	...
	var save = function(docs, type, callback){	
		var esDao = require('../dao/esDao');

		if(type === 'D') { //원문
			logger.info('[documentsSaver] Step#1-1 원문 저장');

			esDao.connect(function(err) {
				if(err){
					callback(err);
				} else {
					esDao.save2ES(docs, TYPE_DOCUMENTS, function(err) {
						if(err) {
							callback(err);
						} else {
							callback(null);
			...
			logger.info('[documentsSaver] Step#1-2 댓글 저장');

			esDao.connect(function(err) {
				if(err){
					callback(err);
				} else {
					esDao.save2ES(docs, TYPE_COMMENTS, function(err) {
						if(err) {
							callback(err);
						} else {
							callback(null);
			...
			callback('ERR_UNKONWN_FILE_TYPE')
		}	
	}; //save
