					...
					async.eachSeries(jsons, function(json, callback) {
						...
						async.waterfall([
							...
							function(docs, callback) {
								logger.info(attachLoggerHeader(ret_num,'Step#5-2 화제어 저장 ' + docs.length));
								crawlDocTopicMaker.process(docs, function(err){
									if (err){
										callback(err);
									} else {
										callback(null);
...
//======================================================================
				...
				if(err) {
						...
						if(err) {
							...
							if (exists){	//존재한다면
								...
								fs.move(sourceProcessingPath + customer + '/' + keyword, sourcePath + customer + '/' + keyword, function(err2){
									logger.debug(sourcePath + customer + '/' + keyword + " 존재 안함");
									if (err2){
										logger.error(err2);
										callback(err2);
									}else{
					...
					/*fs.move(sourceProcessingPath + customer + '/' + keyword, sourcePath + customer + '/' + keyword, function(err2) {
						if(err2) {
							logger.error('[topics] ERR_KEYWORD_FOLDER_MOVE_FAILED ' + err2);
							callback(err);
						} else {
							...
							var slackMessage = {
								color: 'warning',
								title: 'topics',
								value: '[topics] [keyword:' + keyword + '] topics process failed : ' + err
							};
							
							slack.sendMessage(slackMessage, function(slack_err) {
								if (slack_err) {
									logger.warn('[topics] ' + slack_err);
									callback(err);
								} else {
