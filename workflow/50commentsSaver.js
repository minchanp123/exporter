...
const jsonMaker = require('../lib/50jsonMaker_comment');

var commentsSaver = (function () {
			...
			async.waterfall([
								...
								fs.readdir(sourcePath + customer + '/' + keyword, function (err1, scdFolders) {
									if (scdFolders.length === 0) {
										...
										async.eachSeries(scdFolders, function (scdFolder, callback) {
											// comments 폴더의 SCDFOLDER를 PROCESSING으로 이동
				...
				function (jsons, callback) {
					...
					async.eachSeries(jsons, function (json, callback) {
						...
						async.waterfall([
								...
								fs.readJson(json, function (err, docs) {
									if (err) {
										...
										async.eachSeries(docArray, function (doc, callback) {
											...
											if (fileNm.startsWith('C') > -1) {
												logger.info(attachLoggerHeader(ret_num, 'Step#5-1-2 댓글 관련 메타 조회'));

												docsType = 'C';
												jsonMaker.collectJson2crawlJson_v3(doc, function (err, res) {
													if (err) {
														callback(err);
													} else {
														doc = res;
							...
							//Step#5-2 comments array 생성
							function (docs, callback) {
								logger.info(attachLoggerHeader(ret_num, 'Step#5-2 comments array 생성 ' + docs.length));
				...
				if (err) {
					...
					/*var slackMessage = {
						color: 'warning',
						title: 'topics',
						value: '[comments] [keyword:' + keyword + '] topics process failed : ' + err
					};
					
					slack.sendMessage(slackMessage, function(slack_err) {
						if (slack_err) {
							logger.warn('[comments] ' + slack_err);
							callback(err);
						} else {
					...
					/*fs.move(sourceProcessingPath + customer + '/' + keyword, sourcePath + customer + '/' + keyword, function(err2) {
						if(err2) {
							logger.error('[comments] ERR_KEYWORD_FOLDER_MOVE_FAILED ' + err2);
							callback(err);
						} else {
					...
					/*fs.move(sourceProcessingPath + customer + '/' + keyword, sourceEmotionsPath + customer + '/' + keyword, function(err) {
						if(err) {
							logger.error('[comments] ERR_KEYWORD_FOLDER_MOVE_FAILED ' + err);
							callback(err);
						} else {
							callback(null);
						}
	...
	var attachLoggerHeader = function (ret_num, str) {
		return ('[commentsSaver][' + ret_num + '] ' + str);
	} //attachLoggerHeader
